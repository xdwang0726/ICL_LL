...
def main(args):
    ...
    new_datasets = [dataset + "_" + args.variant + (("_" + args.method) if args.method is not None else "") for dataset
                    in datasets]
    ...
    for dataset_idx, (dataset, new_dataset) in enumerate(zip(datasets, new_datasets)):
        ...
        if args.variant == "random_true_distribution":
            ...
            assert os.path.exists(
                full_train_data_path), "Please generate full training data first by running _build_gym.py with k=16384."
            ...
            train_label_distribution = {label: train_label_counter[label] / len(full_train_data_labels) for label in
                                        train_label_counter}
        ...
        for seed in seeds:
            ...
            train_data = []
            train_data_path = os.path.join(args.data_dir, dataset,
                                           "{}_{}_{}_{}.jsonl".format(dataset, args.k, seed, "train"))
            ...
            test_data = []
            test_data_path = os.path.join(args.data_dir, dataset,
                                          "{}_{}_{}_{}.jsonl".format(dataset, args.k, seed, "test"))
            ...
            for dp_idx, dp in enumerate(train_data):
                if args.variant in ["gold", "gold_w_template", "permutated_labels",
                                    "random_english_words_gold_labels"] or \
                    ...
                    dp["output"] = dp["options"][
                        np.random.choice([i for i in range(len(dp["options"])) if dp["options"][i] != dp["output"]])]
                    ...
                    dp["output"] = np.random.choice(list(train_label_distribution.keys()),
                                                    p=list(train_label_distribution.values()))
                else:
                    # assign random label
            ...
            with open(os.path.join(new_dataset_dir, "{}_{}_{}_{}.jsonl".format(new_dataset, args.k, seed, "train")),
                      "w") as f:
                for dp in train_data:
                    f.write(json.dumps(dp))
                    f.write("\n")

            with open(os.path.join(new_dataset_dir, "{}_{}_{}_{}.jsonl".format(new_dataset, args.k, seed, "test")),
                      "w") as f:
                for dp in test_data:
                    f.write(json.dumps(dp))
                    f.write("\n")
